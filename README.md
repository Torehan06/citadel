...
Before M4 (SQS), turn off **System Settings → General → AirDrop & Handoff → AirPlay Receiver**. moto's tests need port 5000, which AirPlay uses on macOS.
...
|---|---|---|
| `CLAUDE_MODELS` | `opus,sonnet` | Model chain, best first. Every session runs on Opus 5.5 until Opus hits its own limit. The loop then continues on the next model instead of sleeping, and goes back to Opus after `STEP_UP_AFTER_MIN` (5 h, one usage window). |
| `CLAUDE_EFFORT` | empty (model default; Opus 5.5 uses `medium`) | `low` to `max`. The biggest lever on burn rate after the model itself. |
| `CLAUDE_SUBAGENT_MODEL` | `sonnet` | Model for any subagents a session spawns. Subagents mostly search and read, and on Opus they'd multiply the burn. Leave it empty to use the session's model. |
...
**Judge models on results.** `harness/report.sh` prints a per-model line: sessions judged, how many improved the scores, how many were reverted, and how many limit hits. After a few nights you'll see whether Opus's extra burn buys more kept work per window than a newer Sonnet does. That's the number to decide on, not the price.

**Usage limits.**
- Subscription plans meter Claude Code in rolling usage windows, and one agent session uses many prompts.
- When every model in the chain is out, the loop sleeps and retries; when it reports a weekly limit, it stops.
- **Keep extra usage off** in claude.ai settings. Otherwise a long Opus night can spill into paid overage.
- **Never set `ANTHROPIC_API_KEY`.** In `-p` mode Claude Code silently prefers it over a subscription login and bills per token. The harness unsets it anyway. Also never use `--bare`: it ignores the subscription login.

**Second lane.** `harness/loop.sh --agent codex` runs Codex CLI on its own separate limits. Its model chain is `CODEX_MODELS` in the same file. Codex reads AGENTS.md natively. Antigravity is an IDE with no headless mode, so it's for daytime interactive work, not the night shift.
...
## Your part (about 5 minutes a morning)
...
3. If `STUCK.md` exists, open `claude --model opus --effort high` in the repo. Unstick it, or split the milestone into smaller steps. Then delete `STUCK.md`, commit, and restart.
4. Once per milestone, have Opus do a code-review pass. The ratchet guarantees behaviour, not code quality.
5. When you're curious, read the `Concept:` lines in PROGRESS.md. Each one is a two-sentence lesson in how AWS actually works.

## Honest expectations

- Rung 1 (the AWS APIs) takes weeks to a few months at this budget, Rung 2 (Terraform) is likely, and Rung 3 (real multi-region) is plausible. Rung 4 (self-hosting) could take the better part of a year of stop-start nights.
- The stuck nights will cluster around SigV4 chunked uploads, the DynamoDB expression grammar and number ordering, and replication. That's normal. Those are the spots that need an interactive session.
